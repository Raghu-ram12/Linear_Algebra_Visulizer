...
class Render:
    ...
    @staticmethod
    def format_vector_components(v: Vector) -> str:
        if v is None:
            return "No vector selected"

        components = getattr(v, "components", [])
        if len(components) >= 3:
            return (
                f"x comp: {components[0]} y comp: {components[1]} z comp: {components[2]}"
            )
        if len(components) == 2:
            return f"x comp: {components[0]} y comp: {components[1]}"
        if len(components) == 1:
            return f"x comp: {components[0]}"
...
class UI(Render):
    ...
    def __init__(self, height, width):
        ...
        self.component_info_frame = None
    ...
    def build_ui(self):
        ...
        self.render_component_display()
        self.update_selected_vector_components()
        self.frame.pack(side="left", expand=True)
        self.canvas.pack(side="right", expand=True)

        self.root.mainloop()
    
    def render_component_display(self):
        self.component_info_frame = tk.Frame(self.frame, width=PANEL_WIDTH)
        self.component_info_frame.pack(fill=tk.X, padx=5, pady=5)

    def update_selected_vector_components(self, event=None):
        if self.component_info_frame is None:
            return

        for widget in self.component_info_frame.winfo_children():
            widget.destroy()

        selection = self.vector_list.curselection() if self.vector_list else ()
        if not selection:
            label = tk.Label(
                self.component_info_frame,
                text="No vector selected",
                justify="left",
        ...
        for idx in selection:
                ...
                label = tk.Label(
                    self.component_info_frame,
                    text=self.format_vector_components(v),
                    justify="left",
                    wraplength=PANEL_WIDTH - 20,
                )
    ...
    def update_slider_value(self, x_theta=None, y_theta=None, z_theta=None):
        ...
        self.redraw()
    def reset_view(self,x_slider,y_slider,z_slider):
        self.theta_x=35.6 
        self.theta_y=45 
        self.theta_z=0 
    ...
    def render_sliders(self):
        ...
        x_label=tk.Label(sliders_frame,text="x-axis")
       
        x_slider = tk.Scale(
            sliders_frame, from_=-90, to=90, orient="horizontal", resolution=1
        )

        y_label=tk.Label(sliders_frame,text="y-axis")
        y_slider = tk.Scale(
            sliders_frame, from_=-90, to=90, orient="horizontal", resolution=1
        )

        z_label=tk.Label(sliders_frame,text="z-axis")
        z_slider = tk.Scale(
            sliders_frame, from_=-90, to=90, orient="horizontal", resolution=1
        )

        reset_btn=tk.Button(sliders_frame,text="Reset View",relief="solid",command=lambda : self.reset_view(x_slider,y_slider,z_slider))
        x_slider.set(self.theta_x)
        y_slider.set(self.theta_y)
        z_slider.set(self.theta_z)
        ...
        zoom_slider=tk.Scale(sliders_frame,from_=20,to=50,orient="horizontal",resolution=1)
        zoom_slider.config(command=lambda v: self.adjust_scale(v))

        x_label.pack(fill=tk.X,padx=5,pady=2)
        x_slider.pack(fill=tk.X, padx=5, pady=2)

        y_label.pack(fill=tk.X,padx=5,pady=2)
        y_slider.pack(fill=tk.X, padx=5, pady=2)

        z_label.pack(fill=tk.X,padx=5,pady=2)
    ...
    def create_new_vector(self, input_frame):
        ...
        if user_entry:
            ...
            index = self.vector_list.size() - 1
            self.vector_list.selection_clear(0, tk.END)
            self.vector_list.selection_set(index)
            self.vector_list.activate(index)
            self.update_selected_vector_components()
            input_frame.destroy()

    def render_vector_list_box(self):

        self.vector_list = tk.Listbox(self.frame, width=30, selectmode=tk.EXTENDED)
        self.vector_list.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.vector_list.bind("<<ListboxSelect>>", self.update_selected_vector_components)
